...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Image analysis endpoint with marketplace integration
  app.post("/api/analyze", upload.single('image'), async (req, res) => {
    try {
      ...
      // Read and convert image for analysis
      const imageBuffer = await fs.readFile(uploadedFile.path);
      const base64Image = imageBuffer.toString('base64');
      const imageHash = generateImageHash(imageBuffer);

      // Check cache first
      const cachedAnalysis = getCachedAnalysis(imageHash);
      if (cachedAnalysis && !hasNegativeFeedback(imageHash)) {
        console.log('Using cached analysis for uploaded image');
        return res.json(cachedAnalysis.analysisData);
      }

      // AI Analysis with Gemini
      const ANALYSIS_PROMPT = `
Analyze this product image and provide detailed information. Respond with only a JSON object:
{
  "productName": "Product name (include brand, model, colorway if visible)",
  "description": "Detailed description of the product and its market characteristics",
  "category": "Product category (shoes, clothing, electronics, etc.)",
  "brand": "Brand name if identifiable",
  "condition": "Estimated condition (new, used, etc.)",
  "estimatedRetailPrice": "Estimated original retail price in USD",
  "marketDemand": "high/medium/low based on visual cues",
...
`;
      ...
      if (productAnalysis.productName && productAnalysis.productName !== "Product analysis failed") {
        ...
        try {
          ...
          const stockxAPI = createStockXProductionAPI();
          if (stockxAPI) {
            const stockxResult = await stockxAPI.searchProducts(productAnalysis.productName);
            if (stockxResult) {
              stockxData = stockxResult;
              if (stockxResult.averagePrice) {
                averagePrice = `$${stockxResult.averagePrice}`;
                resellPrice = `$${Math.round(stockxResult.averagePrice * 0.85)}-$${Math.round(stockxResult.averagePrice * 0.95)}`;
              }
            }
          }
        } catch (error) {
        ...
        try {
          const ebayService = createEbayProductionService();
          if (ebayService) {
            const ebayResult = await ebayService.searchMarketplace(productAnalysis.productName);
            if (ebayResult && ebayResult.averagePrice > 0) {
              ebayData = ebayResult;
              if (!stockxData) {
                averagePrice = `$${ebayResult.averagePrice}`;
                resellPrice = `$${Math.round(ebayResult.averagePrice * 0.8)}-$${Math.round(ebayResult.averagePrice * 0.9)}`;
              }
            }
          }
        } catch (error) {
      ...
      const analysisData = {
        ...
        description: productAnalysis.description || "AI-powered product analysis completed",
      ...
      res.json({ ...analysis, id: analysis.id });
